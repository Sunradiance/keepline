...
class Config:
    SECRET_KEY = os.environ.get('SECRET_KEY', 'keepline-dev-key')
    DEBUG = os.environ.get('FLASK_DEBUG', 'True').lower() == 'true'
    JSON_AS_ASCII = False
    HOST = os.environ.get('KEEPLINE_HOST', '0.0.0.0')
    PORT = int(os.environ.get('KEEPLINE_PORT', '8792'))
    ...
    def validate(cls) -> list[str]:
        ...
        if not cls.LLM_API_KEY:
            warnings.append('LLM_API_KEY not set — AI features disabled')
        if not cls.TAVILY_API_KEY and not cls.SERPER_API_KEY:
            warnings.append('TAVILY_API_KEY / SERPER_API_KEY not set — vendor benchmark scan disabled')
        return warnings